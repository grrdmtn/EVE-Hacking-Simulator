...
- ~~add a menu for easy changing of settings (so that you don't need to change those in the file)~~ - Done!
- add support for custom defenses and utilities
- make a bunch of custom defenses and utilities

## Custom defenses and utilities you say?
...
Also, this minigame may put too much emphasis on strength in it's current state. Defense nodes hit last and cannot hit back if they are dead, which means increasing strength ironically often is better for your coherence than increasing coherence. This makes a *Neural Lace 'Bluefire' Net Ablation 960-10* implant (which sacrifices strength to add coherence) a questionable choice. It could be nice if coherence played more of a role than strength for some nodes.

Lastly I think the minigame could do with some more strategic choices. Often the best thing to do is 'just blindly kill any defense nodes that pop up', especially when you're at 60 strength. Wouldn't it be nice if there was a valid strategic choice between leaving certain defense nodes alive longer or not?
